...
rule summarize_somrit:
    ...
    shell:
        """
        python {params.somrit_test_script} --truth-mat {input.mat_repbase} --truth-pat {input.pat_repbase} --tldr-before {params.tldr_before_txt} --xtea-before {params.xtea_before_txt} --somrit {params.tsv} --fraction {wildcards.frac} --rep {wildcards.rep} > {output.somrit_before_after}
        """
