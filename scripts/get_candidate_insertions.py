        ...
        for idx in range(0, len(aligned_pairs) - 1):
            ...
            if read_gap >= args.min_detected_inclusion_length and ref_gap <= max_ref_gap_at_candidate:
                ...
                if record.is_reverse and insertion_sequence != "":
                    ...
                    read_end = tmp
                records_to_output[record.query_name].append(["%s\t%d\t%d\t%s\t%d\t%d\t%.1f\t%s" % (record.reference_name, ref_start, ref_end, record.query_name, read_start, read_end, orientation, insertion_sequence), annotation])
...
parser.add_argument('--pat-bam', type=str, required=False)
parser.add_argument('--mat-bam', type=str, required=False)
