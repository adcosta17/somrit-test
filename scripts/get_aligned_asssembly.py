...
parser.add_argument('--full-sample', required=True)
...
contigs = {}
contigs[sample] = {}
with pysam.FastxFile(args.assembly_path+'/'+args.full_sample+"/"+args.assembly_folder+"/"+args.full_sample+".mat.fa") as in_mat:
	contigs[sample]["mat"] = defaultdict(str)
	for entry in in_mat:
		contigs[sample]["mat"][entry.name] = entry.sequence
with pysam.FastxFile(args.assembly_path+'/'+args.full_sample+"/"+args.assembly_folder+"/"+args.full_sample+".pat.fa") as in_pat:
...
for hp in ["mat", "pat"]:
	samfile = pysam.AlignmentFile(args.input_path+'/'+sample+"."+hp+".sorted.bam", "rb")
...
for hp in ["mat", "pat"]:
	with open(args.input_path+'/'+sample+"."+hp+".subset.fa", 'w') as out_fa:
